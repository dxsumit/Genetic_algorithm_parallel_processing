...
int main(int argc, char *argv[]) {
    ...
    if (taskid == MASTER){
        ...
        while(!found) {
            ...
            aveseg = s / numworkers;      // average segmentation for each worker....
            extra = s % numworkers;       // number of workers need to do extra work..
            offset = 0;
            // mtype = FROM_MASTER;
            ...
            for(int dest=1; dest<=numworkers; dest++) {
                ...
                MPI_Send(&segment, 1, MPI_INT, dest, FROM_MASTER, MPI_COMM_WORLD);
                MPI_Send(&population[offset], segment, myDataType, dest, FROM_MASTER, MPI_COMM_WORLD);
                MPI_Send(&new_generation[offset], segment, myDataType, dest, FROM_MASTER, MPI_COMM_WORLD);
                offset = offset + segment;

                // MPI_Barrier(MPI_COMM_WORLD);

                cout << "sending " << ten_percent_flag << endl;
            }
            
            // mtype = FROM_WORKER;
            for (int i = 1; i <= numworkers; i++) {
                MPI_Recv(&offset, 1, MPI_INT, i, FROM_WORKER, MPI_COMM_WORLD, &status);
                MPI_Recv(&segment, 1, MPI_INT, i, FROM_WORKER, MPI_COMM_WORLD, &status);
                MPI_Recv(&population[offset], segment, myDataType, i, FROM_WORKER, MPI_COMM_WORLD, &status);
                MPI_Recv(&new_generation[offset], segment, myDataType, i, FROM_WORKER, MPI_COMM_WORLD, &status);

                cout << "received in main " << i << endl;
                // MPI_Barrier(MPI_COMM_WORLD);
            }
            ...
            remaining_population_flag = true;
            // From 50% of fittest population, Individuals will mate to produce offspring
            int remaining_Size = POPULATION_SIZE - s;           //////// 

            aveseg = remaining_Size / numworkers;      // average segmentation for each worker....
            extra = remaining_Size % numworkers;       // number of workers need to do extra work..
            offset = s;                                 // starting index of array..
            mtype = FROM_MASTER;
            ...
            for(int dest=1; dest<=numworkers; dest++) {
                ...
                MPI_Send(&segment, 1, MPI_INT, dest, mtype, MPI_COMM_WORLD);
                MPI_Send(&population[offset], segment, myDataType, dest, mtype, MPI_COMM_WORLD);
                MPI_Send(&new_generation[offset], segment, myDataType, dest, mtype, MPI_COMM_WORLD);
                offset = offset + segment;
            }
            ...
            for (int i = 1; i <= numworkers; i++) {
                ...
                MPI_Recv(&segment, 1, MPI_INT, source, mtype, MPI_COMM_WORLD, &status);
                MPI_Recv(&population[offset], segment, myDataType, source, mtype, MPI_COMM_WORLD, &status);
                MPI_Recv(&new_generation[offset], segment, myDataType, source, mtype, MPI_COMM_WORLD, &status);
            }

            remaining_population_flag = false;
    ...
    //Worker task:
    if (initial_population_flag && taskid > MASTER) {
    ...
    // new 10% creation....
    if(ten_percent_flag && taskid > MASTER){
        ...
        MPI_Recv(&segment, 1, MPI_INT, MASTER, mtype, MPI_COMM_WORLD, &status);
        MPI_Recv(&population, segment, myDataType, MASTER, mtype, MPI_COMM_WORLD, &status);
        MPI_Recv(&new_generation, segment, myDataType, MASTER, mtype, MPI_COMM_WORLD, &status);
        ...
        MPI_Send(&segment, 1, MPI_INT, MASTER, mtype, MPI_COMM_WORLD);
        MPI_Send(&population, segment, myDataType, MASTER, mtype, MPI_COMM_WORLD);
        MPI_Send(&new_generation, segment, myDataType, MASTER, mtype, MPI_COMM_WORLD);


    }


    // remaining population flag..
    if(remaining_population_flag && taskid > MASTER){
